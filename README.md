...
3. **Application ID:** paste the **extension ID** you copied in step 2.
...
`chrome.identity` ties the OAuth client to a specific extension ID, and an
unpacked extension's ID is derived from its path. If the ID ever changes (you
move the folder, or load it elsewhere), update the **Application ID** on the
OAuth client to match — otherwise sign-in fails with a `bad client id` error.
To **pin** the ID, generate a key pair and add a `"key"` field to the manifest
(see Chrome's "keep a consistent extension ID" docs).
